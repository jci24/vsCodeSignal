...
async Task RunGroundTruthScenariosAsync()
{
    ...
    {
        await RunAsync($"Ground truth {scenario.Id}", async () =>
        {
            var composer = CreateResponseComposer(enableLlm: false);
    ...
    AiSummaryCardDto card)
{
    if (!string.Equals(card.Mode, scenario.Expected.Mode, StringComparison.OrdinalIgnoreCase))
        throw new InvalidOperationException($"Expected summary mode '{scenario.Expected.Mode}' but found '{card.Mode}'.");
    ...
    if (!string.Equals(card.RecommendedView ?? string.Empty, scenario.Expected.RecommendedView ?? string.Empty, StringComparison.OrdinalIgnoreCase))
        throw new InvalidOperationException($"Expected recommended view '{scenario.Expected.RecommendedView}' but found '{card.RecommendedView ?? "null"}'.");
    ...
    foreach (var phrase in scenario.Expected.RequiredAnswerPhrases)
    {
        if (!narrative.Answer.Contains(phrase, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Answer did not include required phrase '{phrase}'.");
    }
    ...
    foreach (var phrase in scenario.Expected.RequiredPrimaryFindingPhrases)
    {
        if (!card.PrimaryFinding.Contains(phrase, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Primary finding did not include required phrase '{phrase}'.");
    }
    ...
    foreach (var phrase in scenario.Expected.RequiredImpactPhrases)
    {
        if (!card.ImpactSummary.Contains(phrase, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Impact summary did not include required phrase '{phrase}'.");
    }

    foreach (var phrase in scenario.Expected.ForbiddenAnswerPhrases)
    {
        if (narrative.Answer.Contains(phrase, StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Answer contained forbidden phrase '{phrase}'.");
    }
    ...
    foreach (var code in scenario.Expected.RequiredFactCodes)
    {
        if (!card.KeyFacts.Any(item => string.Equals(item.Code, code, StringComparison.OrdinalIgnoreCase)))
            throw new InvalidOperationException($"Summary card did not include required fact code '{code}'.");
    }
}

static IReadOnlyList<GroundTruthScenario> LoadGroundTruthScenarios()
{
    var path = Path.Combine(AppContext.BaseDirectory, "Fixtures", "guided_compare_ground_truth.json");

    if (!File.Exists(path))
        throw new FileNotFoundException($"Ground-truth fixture file was not found at '{path}'.");

    var json = File.ReadAllText(path);
    var suite = JsonSerializer.Deserialize<GroundTruthSuite>(json, new JsonSerializerOptions(JsonSerializerDefaults.Web)
    {
        PropertyNameCaseInsensitive = true
    });

    if (suite is null || suite.Scenarios.Count == 0)
        throw new InvalidOperationException("Ground-truth fixture file did not contain any scenarios.");
